...
import { getPremiumPlans, getPointPacks, subscribePremium, purchasePack, getMe } from "@/lib/vixor.functions";
...
function Premium() {
  ...
  const packMut = useMutation({
    mutationFn: (id: string) => buy({ data: { packId: id } }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["me"] }),
  });
          ...
          {packs.data?.map(p => (
            <div key={p.id} className={`vixor-card p-4 relative ${p.badge === "Popular" ? "border-primary border-2" : ""}`}>
              ...
              <button onClick={() => packMut.mutate(p.id)} disabled={packMut.isPending}
                className="w-full h-9 rounded-lg bg-muted text-xs font-semibold disabled:opacity-50">
                {packMut.isPending && packMut.variables === p.id ? "…" : `$${(p.price_cents / 100).toFixed(2)}`}
              </button>
            </div>
          ))}
        </div>

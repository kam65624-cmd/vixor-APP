  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button className="size-9 rounded-xl bg-card border border-border flex items-center justify-center"><Filter className="size-4"/></button>
      </div>

      <div className="flex gap-2 overflow-x-auto scrollbar-hide">
        {["All","BUY","SELL","WAIT"].map((f,i) => (
          <button key={f} className={`px-3 h-8 rounded-full text-xs font-semibold whitespace-nowrap ${i === 0 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{f}</button>
        ))}
      </div>
      ...
      <div className="space-y-2">
        ...
        {!q.isLoading && all.length === 0 && (
          <Link to="/analyze" className="vixor-card vixor-card-hover p-6 text-center block">
            ...
            <div className="text-xs text-muted-foreground mt-1">Start your first chart analysis</div>
          </Link>
        )}
        {all.map(a => (
          <Link key={a.id} to="/analysis/$id" params={{ id: a.id }} className="vixor-card vixor-card-hover p-3 flex items-center gap-3">
            ...
            <div className="text-right space-y-1">
              ...
              <div className="text-[10px] text-muted-foreground text-mono">{a.confidence ?? 0}%</div>
            </div>
          </Link>
        ))}
      </div>

...
import { BookOpen, History, Users, Crown, Settings, Bell, Calculator, ChevronRight, LogOut } from "lucide-react";
...
function Profile() {
  ...
  const avatar = (display.split(" ").map(s => s[0]).join("").slice(0, 2) || "U").toUpperCase();
  ...
  const links = [
    { to: "/history", icon: History, label: "Analysis history", desc: "All your past chart analyses" },
    { to: "/referral", icon: Users, label: "Referrals", desc: `${refCount} friend${refCount === 1 ? "" : "s"}` },
    { to: "/premium", icon: Crown, label: "Premium", desc: isPremium ? "Active" : "Unlock unlimited" },
    { to: "/lot-calculator", icon: Calculator, label: "Lot calculator", desc: "Position sizing tool" },
  ...
  return (
    <div className="space-y-5">
      <div className="vixor-card p-5 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-1.5 mt-2">
            {isPremium && <span className="text-[10px] px-2 py-0.5 rounded bg-primary/15 text-primary">Premium</span>}
            <span className="text-[10px] px-2 py-0.5 rounded bg-bullish/15 text-bullish">{me.data?.profile?.streak_days ?? 0}🔥</span>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="vixor-card p-4 grid grid-cols-3 gap-3">
          ...
          <div>
            ...
            <div className="text-xl font-bold text-mono text-bullish">{me.data?.balance.lifetime_earned ?? 0}</div>
          </div>
          <div>
      ...
      <button onClick={signOut} className="w-full h-11 rounded-xl bg-muted text-muted-foreground text-sm font-medium flex items-center justify-center gap-2">

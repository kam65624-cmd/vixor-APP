...
function buildCover() {
  return [
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: "Comprehensive Technical Audit Report",
          size: 32,
          color: c(P.secondary),
          font: { ascii: "Times New Roman" },
        }),
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: "UI/UX, Data Integrity, Architecture & Functional Defects",
          size: 24,
          color: c(P.secondary),
          font: { ascii: "Times New Roman" },
        }),
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: "Prepared for: KIMI K3 Code Flow Agent",
          size: 22,
          color: c(P.body),
          font: { ascii: "Times New Roman" },
        }),
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: "Repository: github.com/kam65624-cmd/vixor-APP",
          size: 20,
          color: c(P.secondary),
          font: { ascii: "Times New Roman" },
        }),
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: "Date: July 19, 2026 | Version: main branch @ commit 1f60e8d",
          size: 20,
          color: c(P.secondary),
          font: { ascii: "Times New Roman" },
        }),
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: "Stack: TanStack Start + React + Vercel + Supabase + Binance WS",
          size: 20,
          color: c(P.secondary),
          font: { ascii: "Times New Roman" },
        }),
    ...
    // ── SECTION 1: TOC ──
    {
      ...
      children: [
        ...
        new Paragraph({
          ...
          children: [
            new TextRun({
              text: "(Right-click the TOC above and select 'Update Field' to refresh page numbers)",
              italics: true,
              size: 18,
              color: "999999",
              font: { ascii: "Times New Roman" },
        ...
        default: new Header({
          children: [
            new Paragraph({
              ...
              children: [
                new TextRun({
                  text: "VIXOR Technical Audit Report",
                  size: 18,
                  color: "808080",
                  font: { ascii: "Times New Roman" },
                }),
      ...
      children: [
        ...
        new Table({
          ...
          rows: [
            ...
            tableRow(
              ["CRITICAL", "Data Integrity / No Real Data", "8", "Home, Discover, Token, Analysis"],
              0,
            ),
            tableRow(["HIGH", "UI/UX Layout & Sizing", "12", "All pages"], 1),
            tableRow(
              ["HIGH", "Missing Functionality", "10", "Charts, Token, Copilot, Discover"],
              2,
            ),
            tableRow(
              ["MEDIUM", "Architecture / Wrong Patterns", "6", "Analysis, Copilot, Data Layer"],
              3,
            ),
        ...
        labeledPara(
          highLabel(),
          "No Built-In Chart Alternative: The app has a CandlestickChart component and lightweight-charts as dependencies, but the charts page only uses TradingView. For environments where TradingView fails, there should be a seamless fallback to the built-in chart engine using Binance kline data.",
        ),
        ...
        labeledPara(
          highLabel(),
          "App-Wide Error Handling Uses Generic Error Page: When any error occurs (like the LINK/USDT AssetRegistry crash), the app shows a full-page 'Something went wrong' error (Screenshot 9, 11, 13) with no context about what went wrong. The RouteErrorBoundary component (src/components/vixor/RouteErrorBoundary.tsx) should show contextual error messages and suggest specific fixes (e.g., 'This token is not supported' vs 'Network error' vs 'Server error').",
        ),
        labeledPara(
          medLabel(),
          "No Offline/Retry Resilience: The app does not implement proper offline detection or retry mechanisms. The useOnline hook exists but is not consistently used. When the server functions fail, there is no automatic retry with exponential backoff for transient failures.",
        ),
        ...
        bullet(
          "Replace getHomeMarketData with CoinGecko free API (/api/v3/coins/markets) for 100+ real tokens with real prices, volumes, and market caps. CoinGecko free tier allows 10-30 requests/minute which is sufficient.",
        ),
        bullet(
          "Add CoinGecko logo URLs to token display. Use https://api.coingecko.com/coins/{id}/image for token logos.",
        ),
        bullet(
          "Integrate DexScreener API (https://api.dexscreener.com/latest/dex/tokens/{address}) for DEX token discovery with real-time prices, volumes, and pair data.",
        ),
        bullet(
          "Connect discover page to DexScreener trending API (/token-boosts/top, /token-profiles/latest) for real-time trending and new token feeds.",
        ),
        bullet(
          "Add Binance REST API fallback (/api/v3/ticker/24hr) for crypto prices when WebSocket is unavailable.",
        ),

        h2("10.2 Phase 2: Fix Charts (Critical)"),
        bullet(
          "Implement lightweight-charts based chart as the primary chart engine (it works in Telegram WebView). Load Binance kline data from /api/v3/klines endpoint. This is already a project dependency.",
        ),
        bullet(
          "Make TradingView chart an optional enhancement that loads only in full browser mode, not in Telegram WebView.",
        ),
        bullet(
          "Auto-generate SYMBOL_MAP from AssetRegistry.tradingViewSymbol() instead of hardcoding.",
        ),
        bullet(
          "On token detail page, show the lightweight-charts chart for both CEX and DEX tokens using appropriate data sources.",
        ),

        h2("10.3 Phase 3: Fix AI Analysis (Critical)"),
        bullet(
          "Set OPENROUTER_MODEL to a vision-capable model (google/gemini-2.5-flash) in Vercel environment. Remove the free model default.",
        ),
        bullet(
          "When vision analysis is unavailable, do NOT fall back to local engine silently. Instead, show a clear message: 'AI vision analysis requires a premium model. Please configure OPENROUTER_MODEL in your deployment settings.'",
        ),
        bullet(
          "Fetch real OHLCV bars from Binance REST API before calling the analysis engine, so even the local fallback has real price data.",
        ),
        bullet(
          "Add chart pair detection validation: after analysis, verify the detected pair matches the selected pair. If they differ, show a warning to the user.",
        ),

        h2("10.4 Phase 4: Fix UI/UX (High)"),
        bullet(
          "Implement a unified design system with consistent spacing (4px grid), sizing (32/40/48px for icons), and typography scale. Audit all components against this system.",
        ),
        bullet("Add MiniSparkline to every token list item on home page and discover page."),
        bullet(
          "Redesign discover page cards to match axiom.trade style: larger cards with logo, sparkline, key metrics in a grid layout.",
        ),
        ...
        bullet(
          "Fix AppShell header: consistent avatar size (40px), wallet pill size, and proper spacing between elements.",
        ),

        h2("10.5 Phase 5: Fix Copilot & Token Detail (High)"),
        bullet(
          "Implement Moxi as the agent layer. The copilot UI should not show model names or act as a raw LLM chat. Instead, show agent personas (Market Analyst, Risk Manager, etc.) and route questions appropriately.",
        ),
        bullet(
          "Inject live context into agent prompts: current prices (from BinanceWS), user portfolio, recent signals, and market overview.",
        ),
        bullet(
          "On token detail page, integrate with DexScreener/Birdeye for: holder distribution chart, recent transactions (buys/sells), liquidity changes, and deployer wallet info.",
        ),
        bullet(
          "Add real on-chain data: transaction count, unique holders, top holders percentage, liquidity locked, volume by DEX.",
        ),
          ...
          rows: [
            ...
            tableRow(
              [
                ...
                "Home Page",
                "Mock data, no sparklines, static widgets, sizing",
              ],
              0,
            ),
            tableRow(
              [
                "src/components/vixor/AppShell.tsx",
                "App Shell",
                "Header sizing, nav misalignment, wallet pill",
              ],
              1,
            ),
            tableRow(
              [
                "src/routes/_authenticated/charts.tsx",
                "Charts Page",
                "No fallback chart, limited symbols",
              ],
              2,
            ),
            tableRow(
              [
                "src/components/vixor/TradingViewChart.tsx",
                "TV Chart Widget",
                "Hardcoded SYMBOL_MAP, no WV fallback",
              ],
              3,
            ),
            tableRow(
              [
                "src/domains/analysis/server/run-analysis.ts",
                "AI Analysis",
                "Wrong model, no vision, fabricated results",
              ],
              4,
            ),
            tableRow(
              [
                "src/routes/_authenticated/-copilot-component.tsx",
                "AI Copilot",
                "Raw LLM chat, no Moxi, no context",
              ],
              5,
            ),
            tableRow(
              [
                ...
                "Discover Page",
                "Static data, no DEX integration, bad cards",
              ],
              6,
            ),
            tableRow(
              [
                "src/routes/_authenticated/-token-symbol-component.tsx",
                "Token Detail",
                "No chart, no trades, no holders, empty space",
              ],
              7,
            ),
            tableRow(
              [
                "src/shared/asset-registry/types.ts",
                "Asset Registry",
                "Hardcoded pairs, bottleneck, LINK missing (fixed)",
              ],
              8,
            ),
            tableRow(
              [
                "src/shared/market-data/use-live-prices.ts",
                "Live Prices Hook",
                "Registry-dependent, no dynamic resolution",
              ],
              9,
            ),
            tableRow(
              [
                "src/shared/market-data/binance-ws.ts",
                "Binance WebSocket",
                "Only registered pairs, no DEX support",
              ],
              10,
            ),
          ],
        ...
        body(
          "Below are precise, copy-paste-ready prompts for KIMI K3 to execute each fix. Each prompt is self-contained with all necessary context.",
        ),
        ...
        code(
          "   Map the response to HomeMarketData interface. Use 'sparkline_in_7d.price' for sparkline data.",
        ),
        ...
        code(""),
        code(
          "2. Create a BinanceKlineChart component using lightweight-charts (already installed):",
        ),
        code(
          "   - Fetch klines from Binance REST: GET /api/v3/klines?symbol=BTCUSDT&interval=1h&limit=200",
        ),
        ...
        code(
          "   environment variable. Set it to a vision-capable model like google/gemini-2.5-flash'",
        ),
        code(""),
        code("2. Before calling generateObject, fetch real OHLCV bars from Binance:"),
        code(
          "   GET https://api.binance.com/api/v3/klines?symbol={binanceSymbol}&interval=1h&limit=100",
        ),
        ...
        code(
          "   Step 1: 'Uploading chart...' Step 2: 'AI is analyzing...' Step 3: 'Generating report...'",
        ),

...
function Dashboard() {
  ...
  const recent = useQuery({
    queryKey: ["analyses", 5],
    queryFn: () => fetchList({ data: { limit: 5 } }),
  });
    ...
    <div className="space-y-5">
      ...
      <div className="vixor-card p-4 vixor-card-hover">
        <div className="flex items-center justify-between">
          ...
          <button className="px-3 h-8 rounded-lg bg-primary text-primary-foreground text-xs font-semibold">
            Claim
          </button>
        </div>
        <div className="flex items-center gap-1.5 mt-3">
          {[1, 2, 3, 4, 5, 6, 7].map((d) => (
            <div
              key={d}
              className={`flex-1 h-1.5 rounded-full ${d <= streak ? "bg-primary" : "bg-muted"}`}
            />
          ))}
        </div>
      </div>
      ...
      {!isPremium && (
        <Link
          to="/premium"
          className="block vixor-card p-4 vixor-card-hover relative overflow-hidden"
        >
          ...
          {quickActions.map((qa) => {
            ...
            return (
              <Link
                key={qa.to}
                to={qa.to}
                className="vixor-card vixor-card-hover p-4 flex flex-col gap-3"
              >
                <div
                  className={`size-10 rounded-xl flex items-center justify-center ${accents[qa.accent]}`}
                >
          ...
          {!recent.isLoading && (recent.data?.length ?? 0) === 0 && (
            <Link to="/analyze" className="vixor-card vixor-card-hover p-6 text-center block">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                Drop your first chart to get started
              </div>
            </Link>
          )}
        ...
        <SectionTitle
          ...
          action={
            <span className="text-xs text-muted-foreground">
              {achievementDefs.filter((a) => a.req(analysesCount, streak)).length} /{" "}
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
          {achievementDefs.map((a) => {
            ...
            return (
              <div
                key={a.id}
                className={`vixor-card p-3 min-w-[120px] flex flex-col items-center gap-2 ${!unlocked && "opacity-40"}`}
              >
                <div
                  className={`size-10 rounded-xl flex items-center justify-center ${unlocked ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}
                >

...
import { c as createRouter, a as createRootRouteWithContext, L as Link, u as useNavigate, O as Outlet, H as HeadContent, S as Scripts, b as createFileRoute, l as lazyRouteComponent, d as useLocation } from "../_libs/tanstack__react-router.mjs";
import { S as redirect } from "../_libs/tanstack__router-core.mjs";
import { j as jsxRuntimeExports, r as reactExports } from "../_libs/react.mjs";
import { c as createServerFn, T as TSS_SERVER_FUNCTION, g as getServerFnById } from "./server-CwDusuaO.mjs";
import { r as requireSupabaseAuth } from "./auth-middleware-CksiIeMT.mjs";
...
const appCss = "/assets/styles-CtO7rrrI.css";
const slides = [
...
createServerFn({
  ...
  symbol: string().min(1)
}).parse(d)).handler(createSsrRpc("42ad4fa8fac4f53c13979fd2de764a0d7673abae5a07b08e1128bf3d49aff281"));
createServerFn({
  method: "GET"
}).validator((d) => object({
  symbol: string().min(1),
  amount: number().positive()
}).parse(d)).handler(createSsrRpc("3704c4ae5dc84daff150b56423fdce5e5bdeeb750b6dad59cd4427b9594feee9"));
createServerFn({
  method: "GET"
}).validator((d) => object({
...
}).parse(d ?? {})).handler(createSsrRpc("a993c7a7b00b2e84385012d692044a9a86734f381b116787164a2d533378bdb5"));
createServerFn({
  method: "GET"
}).validator((d) => object({
  symbol: string().min(1)
}).parse(d)).handler(createSsrRpc("14e3ec1e54b76e21b015af136401f08e3b4e04333fe72e8e4dc7e4da13483d18"));
createServerFn({
  method: "GET"
}).validator((d) => object({
  symbol: string().min(1)
}).parse(d)).handler(createSsrRpc("097624e72a52651a46efbca692ba9e81c94030a4487fcef364565d2b61f186dc"));
createServerFn({
  method: "GET"
}).validator((d) => object({
  symbol: string().min(1)
}).parse(d)).handler(createSsrRpc("37a41c096bde66f0a44fc7c5139c720d8b02237d98d39d65e2a49d622a983ab7"));
createServerFn({
  method: "GET"
}).validator((d) => object({
  symbol: string().min(1),
...
}).parse(d)).handler(createSsrRpc("f6aecc3762761e217bda8ebeb2e88f2f38c5b1ed41b817b9e87d0bdfb905ad7f"));
createServerFn({
  method: "GET"
}).validator((d) => object({
  symbol: string().min(1)
}).parse(d)).handler(createSsrRpc("129b4ee99e0e545d6235f131340b9ef036b8e1573adc6023a38b20723782cc6b"));
createServerFn({
  method: "GET"
}).validator((d) => object({
  symbol: string().min(1)
}).parse(d)).handler(createSsrRpc("43c431402a1e938a0fa881732eba5f430f2f20d699a1a20d146747fa8b9791a4"));
createServerFn({
  method: "GET"
}).validator((d) => object({
  symbol: string().min(1)
}).parse(d)).handler(createSsrRpc("0a4880e8d7693e27bb440efbdfef044c9d241436791a926b82a53d1cc80474bd"));
createServerFn({
  method: "GET"
}).validator((d) => object({
  symbol: string().min(1)
}).parse(d)).handler(createSsrRpc("d33a41d5f3dbd0fa5426a4de7573d8ae1abd7cfc2651518646902dff486305bf"));
function useStableServerFn(fn) {
  const fnRef = reactExports.useRef(fn);
  fnRef.current = fn;
  const stableRef = reactExports.useRef(null);
  if (!stableRef.current) {
    stableRef.current = (...args) => fnRef.current(...args);
  }
  return stableRef.current;
}
const MAX_RAPID_RENDERS = 20;
const RESET_INTERVAL = 1e3;
let _renderLoopDetected = false;
let _renderLoopComponent = "";
function wasRenderLoopDetected() {
  return _renderLoopDetected;
}
function getRenderLoopComponent() {
  return _renderLoopComponent;
}
function clearRenderLoopFlag() {
  _renderLoopDetected = false;
  _renderLoopComponent = "";
}
function useRenderGuard(componentName) {
  const renderCount = reactExports.useRef(0);
  const lastRenderTime = reactExports.useRef(Date.now());
  const resetTimer = reactExports.useRef(null);
  renderCount.current++;
  const now = Date.now();
  ...
  if (renderCount.current >= MAX_RAPID_RENDERS) {
    console.error(
      `[Vixor Render Guard] Component "${componentName}" has rendered ${renderCount.current} times in rapid succession. This is likely causing React error #310. Check for setState calls during render or unstable dependencies.`
    );
...
function AppShell({ children }) {
  ...
  const telegramLinkedRef = reactExports.useRef(false);
  reactExports.useEffect(() => {
    if (typeof window === "undefined") return;
  ...
  const closeOnboarding = reactExports.useCallback(() => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-h-screen bg-background text-foreground flex flex-col", children: [
    ...
    showOnboarding && /* @__PURE__ */ jsxRuntimeExports.jsx(OnboardingModal, { onClose: closeOnboarding })
  ] });
}
...
class GlobalErrorBoundary extends reactExports.Component {
  ...
  render() {
    if (this.state.hasError) {
      return /* @__PURE__ */ jsxRuntimeExports.jsx(ErrorView, { error: this.state.error, onReset: this.handleReset });
    }
...
function ErrorView({ error, onReset }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex min-h-screen items-center justify-center bg-background px-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-md text-center", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "size-16 rounded-2xl bg-bearish/10 flex items-center justify-center mx-auto mb-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { className: "size-8 text-bearish", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-2 text-sm text-muted-foreground", children: error?.message?.includes("#310") || wasRenderLoopDetected() ? `A rendering loop was detected${wasRenderLoopDetected() ? ` in ${getRenderLoopComponent()}` : ""}. This has been automatically resolved.` : error?.message ?? "An unexpected error occurred." }),
...
function RootComponent() {
  ...
  reactExports.useEffect(() => {
    ...
    import("./client-Lj_VRnUR.mjs").then(({ supabase: supabase2 }) => {
      ...
      const { data: sub } = supabase2.auth.onAuthStateChange((event) => {
        ...
        const now = Date.now();
        if (event === lastAuthEvent && now - lastAuthTime < 2e3) return;
        lastAuthEvent = event;
        lastAuthTime = now;
  ...
  const handleErrorReset = reactExports.useCallback(() => {
    queryClientRef.current.clear();
    navigateRef.current({ to: "/", replace: true });
  }, []);
  return /* @__PURE__ */ jsxRuntimeExports.jsx(QueryClientProvider, { client: queryClient, children: /* @__PURE__ */ jsxRuntimeExports.jsx(GlobalErrorBoundary, { onReset: handleErrorReset, children: /* @__PURE__ */ jsxRuntimeExports.jsx(AppShell, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(Outlet, {}) }) }) });
}
const $$splitComponentImporter$e = () => import("./auth-yNOJi9Iu.mjs");
...
const $$splitComponentImporter$c = () => import("./index-C5SDP_sR.mjs");
...
const $$splitComponentImporter$a = () => import("./signals-Bt1MeMaY.mjs");
...
const $$splitComponentImporter$9 = () => import("./settings-DZlNMrzW.mjs");
...
const $$splitComponentImporter$8 = () => import("./referral-CZOVve-Y.mjs");
...
const $$splitComponentImporter$7 = () => import("./profile-CcU4jg2p.mjs");
...
const $$splitComponentImporter$6 = () => import("./premium-GoiUTgYV.mjs");
...
const $$splitComponentImporter$5 = () => import("./notifications-CQ7W7Wr1.mjs");
...
const $$splitComponentImporter$4 = () => import("./journal-PTZcK0HJ.mjs");
...
const $$splitComponentImporter$3 = () => import("./discover-BgShSltd.mjs");
...
const $$splitComponentImporter$2 = () => import("./charts-B8YqlnSE.mjs");
...
const $$splitComponentImporter$1 = () => import("./analyze-Pt0MTcvs.mjs");
...
const $$splitComponentImporter = () => import("./analysis._id-BI5888gn.mjs");

...
import { u as useServerFn, j as getMarketNews } from "./router-D5kljJ2o.mjs";
import { S as Skeleton } from "./skeleton-EJ9BKDj5.mjs";
import "../_libs/seroval.mjs";
import { C as Compass, R as Search, W as Flame, X as Layers, k as ArrowUpRight, l as ArrowDownRight, j as ChartNoAxesColumn, Y as ExternalLink } from "../_libs/lucide-react.mjs";
...
function DiscoverNews() {
  ...
  const fetchMarketNewsRef = reactExports.useRef(fetchMarketNews);
  fetchMarketNewsRef.current = fetchMarketNews;
  const [category, setCategory] = reactExports.useState("forex");
  const newsQueryFn = reactExports.useCallback(async () => fetchMarketNewsRef.current({
    data: {
      category
    }
  }), [category]);

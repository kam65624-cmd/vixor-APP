...
import { u as useServerFn, h as listNotifications, m as markAllNotificationsRead } from "./router-D5kljJ2o.mjs";
import { a as useQueryClient, u as useQuery, b as useMutation } from "../_libs/tanstack__react-query.mjs";
import "../_libs/seroval.mjs";
import { n as ArrowLeft, Q as Check, S as Sparkles, v as Users, G as Gift, T as TrendingUp } from "../_libs/lucide-react.mjs";
...
function NotificationsPage() {
  ...
  const fetchRef = reactExports.useRef(fetch);
  fetchRef.current = fetch;
  const markAllRef = reactExports.useRef(markAll);
  markAllRef.current = markAll;
  const notifsQueryFn = reactExports.useCallback(async () => fetchRef.current({}), []);
  const q = useQuery({
    queryKey: ["notifs"],
    queryFn: notifsQueryFn
  });
  const m = useMutation({
    mutationFn: () => markAllRef.current({}),

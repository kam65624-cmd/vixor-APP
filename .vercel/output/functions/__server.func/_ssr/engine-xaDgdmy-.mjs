...
function runLocalAnalysis(input) {
  ...
  const buySideLiquidity = liquidityZones.filter((z) => z.type === "BUY_SIDE").sort((a, b) => b.strength - a.strength).slice(0, 3).map((z) => formatPrice(z.price, decimals));
  const sellSideLiquidity = liquidityZones.filter((z) => z.type === "SELL_SIDE").sort((a, b) => b.strength - a.strength).slice(0, 3).map((z) => formatPrice(z.price, decimals));

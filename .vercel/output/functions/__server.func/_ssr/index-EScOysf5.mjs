...
import { u as useServerFn, g as getMe, l as listAnalyses, a as getMarketPrices, b as getDailySignals, d as listAlerts } from "./router-dhCdxqZe.mjs";
...
function CommandCenter() {
  ...
  const fetchRecentRef = reactExports.useRef(fetchRecent);
  fetchRecentRef.current = fetchRecent;
  const fetchPricesRef = reactExports.useRef(fetchPrices);
  fetchPricesRef.current = fetchPrices;
  const fetchSignalsRef = reactExports.useRef(fetchSignals);
  fetchSignalsRef.current = fetchSignals;
  const fetchAlertsRef = reactExports.useRef(fetchAlerts);
  fetchAlertsRef.current = fetchAlerts;
  const meQueryFn = reactExports.useCallback(async () => fetchMeRef.current({}), []);
  const recentQueryFn = reactExports.useCallback(async () => fetchRecentRef.current({
    data: {
      limit: 5
    }
  }), []);
  const pricesQueryFn = reactExports.useCallback(async () => fetchPricesRef.current({}), []);
  const signalsQueryFn = reactExports.useCallback(async () => fetchSignalsRef.current({
    data: {}
  }), []);
  const alertsQueryFn = reactExports.useCallback(async () => fetchAlertsRef.current({
    data: {}
  }), []);
  const me = useQuery({
    queryKey: ["me"],

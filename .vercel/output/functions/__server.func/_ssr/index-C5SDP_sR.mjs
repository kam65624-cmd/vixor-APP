...
import { a as useRenderGuard, u as useStableServerFn, g as getMe, l as listAnalyses, b as getMarketPrices, d as getDailySignals, e as listAlerts } from "./router-DMFZ8c9Z.mjs";
...
import "./server-CwDusuaO.mjs";
import "node:async_hooks";
import "../_libs/h3-v2.mjs";
import "../_libs/rou3.mjs";
import "../_libs/srvx.mjs";
import "./auth-middleware-CksiIeMT.mjs";
...
function CommandCenter() {
  ...
  const me = useQuery(reactExports.useMemo(() => ({
    queryKey: ["me"],
    queryFn: () => fetchMe({})
  }), [fetchMe]));
  useQuery(reactExports.useMemo(() => ({
    queryKey: ["analyses", 5],
    queryFn: () => fetchRecent({
      data: {
        limit: 5
  ...
  const prices = useQuery(reactExports.useMemo(() => ({
    queryKey: ["market-prices"],
    queryFn: () => fetchPrices({}),
    staleTime: 6e4,
    refetchInterval: 12e4
  }), [fetchPrices]));
  const signals = useQuery(reactExports.useMemo(() => ({
  ...
  const alerts = useQuery(reactExports.useMemo(() => ({

...
import { u as useServerFn, b as getDailySignals, e as getUserStrategy, f as generateDailySignals, h as createAlert, i as updateUserStrategy } from "./router-dhCdxqZe.mjs";
...
function DailySignals() {
  ...
  const signalsRef = reactExports.useRef(signalsFn);
  signalsRef.current = signalsFn;
  const signalsQueryFn = reactExports.useCallback(async () => signalsRef.current({
    data: {}
  }), []);
  ...
  const strategyRef = reactExports.useRef(strategyFn);
  strategyRef.current = strategyFn;
  const strategyQueryFn = reactExports.useCallback(async () => strategyRef.current({}), []);
  ...
  const generateRef = reactExports.useRef(generateFn);
  ...
  const createAlertRef = reactExports.useRef(createAlertFn);
  createAlertRef.current = createAlertFn;
  const alertMutation = useMutation({
    mutationFn: async (data) => createAlertRef.current({
      data: {
...
function StrategyConfig({
  ...
  const updateRef = reactExports.useRef(updateFn);

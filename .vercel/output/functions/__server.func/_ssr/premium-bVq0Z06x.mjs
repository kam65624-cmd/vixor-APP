...
import { u as useServerFn, d as getPremiumPlans, e as getPointPacks, g as getMe, s as subscribePremium, p as purchasePack, f as createStarsInvoice, i as isInsideTelegram, o as openTelegramInvoice } from "./router-D5kljJ2o.mjs";
import { a as useQueryClient, u as useQuery, b as useMutation } from "../_libs/tanstack__react-query.mjs";
import "../_libs/seroval.mjs";
import { n as ArrowLeft, y as Crown, S as Sparkles, Z as Zap, a as ChartColumn, b as Bell, O as ShieldCheck, Q as Check, g as LoaderCircle } from "../_libs/lucide-react.mjs";
...
import "./server-CCzWFmWl.mjs";
import "node:async_hooks";
import "../_libs/h3-v2.mjs";
import "../_libs/rou3.mjs";
import "../_libs/srvx.mjs";
import "./auth-middleware-1P-qMUgE.mjs";
...
function Premium() {
  ...
  const fetchPlansRef = reactExports.useRef(fetchPlans);
  fetchPlansRef.current = fetchPlans;
  const fetchPacksRef = reactExports.useRef(fetchPacks);
  fetchPacksRef.current = fetchPacks;
  ...
  const buyRef = reactExports.useRef(buy);
  buyRef.current = buy;
  const buyStarsRef = reactExports.useRef(buyStars);
  buyStarsRef.current = buyStars;
  const plansQueryFn = reactExports.useCallback(async () => fetchPlansRef.current({}), []);
  const packsQueryFn = reactExports.useCallback(async () => fetchPacksRef.current({}), []);
  const meQueryFn = reactExports.useCallback(async () => fetchMeRef.current({}), []);
  const plans = useQuery({
    queryKey: ["plans"],
    queryFn: plansQueryFn
  });
  const packs = useQuery({
    queryKey: ["packs"],
    queryFn: packsQueryFn
  });
  const me = useQuery({
    queryKey: ["me"],
    queryFn: meQueryFn
  ...
  const subMut = useMutation({
    mutationFn: (id) => subscribeRef.current({
      data: {
        planId: id
      }
    }),
  ...
  const packMut = useMutation({
    mutationFn: async (id) => {
      if (isInsideTelegram()) {
        ...
        const res = await buyStarsRef.current({
          data: {
            packId: id,
            amountStars
          }

...
import { u as useServerFn, a as getMarketPrices, d as listAlerts, x as deleteAlert, h as createAlert } from "./router-dhCdxqZe.mjs";
...
function AlertsList({ pair, onRefresh }) {
  ...
  const listAlertsRef = reactExports.useRef(listAlertsFn);
  listAlertsRef.current = listAlertsFn;
  const deleteAlertRef = reactExports.useRef(deleteAlertFn);
  deleteAlertRef.current = deleteAlertFn;
  const queryFn = reactExports.useCallback(async () => {
    return listAlertsRef.current({ data: { pair, status: void 0 } });
  }, [pair]);
  ...
  const handleDelete = reactExports.useCallback(async (alertId) => {
    try {

import { c as createServerRpc } from "./createServerRpc-DjwVl7f4.mjs";
  ...
  try {
    ...
    try {
      ...
      if (pair.includes("USDT") || pair.includes("BTC") || pair.includes("ETH") || pair.includes("SOL")) {
        const {
          fetchBinanceKlines
        } = await import("./price-fetcher.server-D-w1r8EK.mjs");
...
const runAlertCheck = createServerFn({
  ...
  const {
    checkAllAlerts
  } = await import("./alert-checker.server-CnuxcZCd.mjs");

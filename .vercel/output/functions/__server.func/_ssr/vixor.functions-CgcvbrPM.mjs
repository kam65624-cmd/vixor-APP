import { c as createServerRpc } from "./createServerRpc-2Od2N9yE.mjs";
import { c as createServerFn } from "./server-CwDusuaO.mjs";
  ...
  try {
    ...
    try {
      ...
      if (pair.includes("USDT") || pair.includes("BTC") || pair.includes("ETH") || pair.includes("SOL")) {
        const {
          fetchBinanceKlines
        } = await import("./price-fetcher.server-B6Qlu9qv.mjs");
...
const runAlertCheck = createServerFn({
  ...
  const {
    checkAllAlerts
  } = await import("./alert-checker.server-D0JoRale.mjs");
...
const getExchangeRate = createServerFn({
  ...
  const {
    fetchExchangeRate
  } = await import("./twelvedata.server-ab0rclAn.mjs");

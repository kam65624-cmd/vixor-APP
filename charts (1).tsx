      ...
      <div className="vixor-card p-4">
        <div className="flex items-start justify-between">
          <div>
            ...
            <div
              className={`text-sm font-semibold text-mono ${sym.change >= 0 ? "text-bullish" : "text-bearish"}`}
            >
              {sym.change >= 0 ? "+" : ""}
              {sym.change}%
            </div>
        ...
        <div className="h-44 mt-3">
          <svg className="w-full h-full" viewBox="0 0 400 180" preserveAspectRatio="none">
            ...
            {[40, 80, 120].map((y) => (
              <line key={y} x1="0" x2="400" y1={y} y2={y} stroke="oklch(1 0 0 / 0.05)" />
            ))}
            <path
              d="M 0 140 L 30 130 L 60 135 L 90 110 L 120 115 L 150 90 L 180 100 L 210 70 L 240 80 L 270 50 L 300 65 L 330 40 L 360 55 L 400 35 L 400 180 L 0 180 Z"
              fill="url(#cg)"
            />
            <path
              d="M 0 140 L 30 130 L 60 135 L 90 110 L 120 115 L 150 90 L 180 100 L 210 70 L 240 80 L 270 50 L 300 65 L 330 40 L 360 55 L 400 35"
              fill="none"
        ...
        <div className="vixor-card divide-y divide-border">
          {watchlist.map((w) => (
            <button
              key={w.pair}
              onClick={() => setSym(w)}
              className="w-full p-3 flex items-center gap-3 vixor-card-hover"
            >
              ...
              <div className="text-right">
                ...
                <div
                  className={`text-xs text-mono ${w.change >= 0 ? "text-bullish" : "text-bearish"}`}
                >
                  {w.change >= 0 ? "+" : ""}
                  {w.change}%
                </div>

...
import {
  BookOpen, Users, Crown, Settings, Bell,
...
const BADGES = [
  { id: "first_analysis", icon: "🎯", label: "First Analysis", desc: "Completed your first chart read" },
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between items-center">
        <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Level {level}</span>
        <span className="text-[10px] font-bold text-muted-foreground">{xp} / {level * 100} XP</span>
      </div>
...
function Profile() {
  ...
  const avatarFallback = (display.split(" ").map((s: string) => s[0]).join("").slice(0, 2) || "U").toUpperCase();
  const isPremium = !!me.data?.isPremium;
  const refCount = refs.data?.count ?? 0;
  const joinedAt = me.data?.profile?.created_at;
  const joinedDays = joinedAt ? Math.floor((Date.now() - new Date(joinedAt).getTime()) / 86400000) : 0;
    ...
    <div className="space-y-5 pb-8">
      ...
      <div className="vixor-card p-5">
        <div className="flex items-center gap-4 mb-4">
          <div className="relative">
            {tgPhoto ? (
              <img src={tgPhoto} alt="Telegram Avatar" className="size-18 rounded-2xl border border-border object-cover" />
            ) : (
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          ...
          <div key={s.label} className="vixor-card p-3 text-center">
            ...
            <div className="text-[8px] uppercase font-bold text-muted-foreground mt-0.5">{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {BADGES.map(b => {
            ...
            return (
              <div key={b.id} className={`vixor-card p-3 flex items-center gap-3 transition-opacity ${unlocked ? "opacity-100" : "opacity-40"}`}>
                <div className="text-2xl">{b.icon}</div>
                <div className="min-w-0">
                  <div className="font-bold text-xs leading-none mb-0.5">{b.label}</div>
                  <div className="text-[9px] text-muted-foreground leading-tight line-clamp-2">{b.desc}</div>
                </div>
      ...
      <div className="vixor-card divide-y divide-border overflow-hidden">
        {links.map(l => {
          const Icon = l.icon;
          return (
            <Link key={l.to} to={l.to} className="p-3.5 flex items-center gap-3 hover:bg-card-hover transition-colors group">
              <div className={`size-9 rounded-xl flex items-center justify-center shrink-0 ${l.color}`}>

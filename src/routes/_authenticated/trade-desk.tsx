  ...
  return (
    <div className="space-y-6 pb-6 animate-in fade-in duration-500">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-tight leading-none">{t("tradeDesk.tradeDesk")}</h1>
          <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mt-1">{t("tradeDesk.institutionalExecution")}</div>
        </div>
      </div>
      ...
      <div className="vixor-card p-5 border-l-4 border-l-primary">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-bold text-[11px] uppercase tracking-widest text-muted-foreground">{t("tradeDesk.riskCalculator")}</h2>
        </div>

        <div className="grid grid-cols-2 gap-3 mb-5">
          <div className="col-span-2 space-y-1.5">
            <label className="text-[10px] uppercase font-bold text-muted-foreground">{t("tradeDesk.tradingPair")}</label>
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] uppercase font-bold text-muted-foreground">{t("tradeDesk.balance")} ($)</label>
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] uppercase font-bold text-muted-foreground">{t("tradeDesk.riskPct")}</label>
          ...
          <div className="col-span-2 space-y-1.5">
            <label className="text-[10px] uppercase font-bold text-muted-foreground">{t("tradeDesk.stopLossPips")}</label>
        ...
        <div className="bg-card-hover p-4 rounded-xl border border-border text-center">
          <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-1">{t("tradeDesk.recommendedLotSize")}</div>
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            {result && (
              <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase ${result.riskLevel === "LOW" ? "bg-bullish/10 text-bullish" : result.riskLevel === "MEDIUM" ? "bg-neutral-wait/10 text-neutral-wait" : "bg-bearish/10 text-bearish"}`}>
                {result.riskLevel === "LOW" ? t("tradeDesk.lowRisk") : result.riskLevel === "MEDIUM" ? t("tradeDesk.mediumRisk") : t("tradeDesk.highRisk")}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 px-1">
          ...
          <h2 className="font-bold text-[11px] uppercase tracking-widest text-muted-foreground">{t("tradeDesk.activePositions")}</h2>
        </div>

...
export function determineTrendFromSwings(
  ...
  const highStructure =
    hhCount > lhCount ? "HIGHER_HIGHS" : lhCount > hhCount ? "LOWER_HIGHS" : "MIXED_HIGHS";
  const lowStructure =
    hlCount > llCount ? "HIGHER_LOWS" : llCount > hlCount ? "LOWER_LOWS" : "MIXED_LOWS";
  ...
  for (let i = 0; i < bars.length; i++) {
    ...
    for (const sh of sortedHighs) {
      ...
      if (bar.close > sh.price) {
        ...
        const eventType: "BOS" | "CHoCH" = currentTrend === "BULLISH" ? "BOS" : "CHoCH";
    ...
    for (const sl of sortedLows) {
      ...
      if (bar.close < sl.price) {
        ...
        const eventType: "BOS" | "CHoCH" = currentTrend === "BEARISH" ? "BOS" : "CHoCH";
    ...
    if (grouped.length === 1) {
      ...
      if (choch) {
        ...
        const sorted = [...grouped].sort(
          (a: BOSEvent, b: BOSEvent) => Math.abs(b.price) - Math.abs(a.price),
        );

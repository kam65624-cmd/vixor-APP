 ...
 */
export function detectSRLevels(bars: OHLCVBar[], lookback = 50, tolerance = 0.003): SRLevel[] {
  ...
  const levels: SRLevel[] = clusters.map((cluster) => {
    ...
    const type: "SUPPORT" | "RESISTANCE" | "PIVOT" =
      distance < 0.002 ? "PIVOT" : avgPrice < currentPrice ? "SUPPORT" : "RESISTANCE";
 ...
 */
function clusterByPrice(points: SwingPoint[], tolerance: number): SwingPoint[][] {
  if (points.length === 0) return [];

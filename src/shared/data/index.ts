...
let forexCache: { data: ForexPriceItem[]; fetchedAt: number } | null = null;

export const getLiveForexPrices = createServerFn({ method: "GET" }).handler(async () => {
  if (forexCache && Date.now() - forexCache.fetchedAt < 60_000) {
    return forexCache.data;
  }

  try {
    // Frankfurter API: free, no key, ECB rates
    // Gold is not supported by Frankfurter — we'll use a fallback price
    // Fetch rates with USD as base (for pairs like EUR/USD, GBP/USD)
    const usdRes = await fetch("https://api.frankfurter.app/latest?from=USD");
    const usdRates: Record<string, number> = usdRes.ok
      ? ((await usdRes.json()) as { rates: Record<string, number> }).rates
      : {};

    // Fetch rates with EUR as base (for cross pairs like EUR/GBP, EUR/JPY)
    const eurRes = await fetch("https://api.frankfurter.app/latest?from=EUR");
    const eurRates: Record<string, number> = eurRes.ok
      ? ((await eurRes.json()) as { rates: Record<string, number> }).rates
      : {};

    // Fetch rates with GBP as base (for GBP/JPY, GBP/AUD)
    const gbpRes = await fetch("https://api.frankfurter.app/latest?from=GBP");
    const gbpRates: Record<string, number> = gbpRes.ok
      ? ((await gbpRes.json()) as { rates: Record<string, number> }).rates
      : {};

    const priceMap: Record<string, number> = {
      // USD-based
      "EUR/USD": usdRates["EUR"] ?? 1.0847,
      "GBP/USD": usdRates["GBP"] ?? 1.2715,
      "USD/JPY": usdRates["JPY"] ?? 154.32,
      "USD/CHF": usdRates["CHF"] ?? 0.8823,
      "AUD/USD": usdRates["AUD"] ?? 0.6538,
      "NZD/USD": usdRates["NZD"] ?? 0.5912,
      "USD/CAD": usdRates["CAD"] ?? 1.3654,
      // EUR-based
      "EUR/GBP": eurRates["GBP"] ?? 0.8532,
      "EUR/JPY": eurRates["JPY"] ?? 167.48,
      "EUR/AUD": eurRates["AUD"] ?? 1.6593,
      // GBP-based
      "GBP/JPY": gbpRates["JPY"] ?? 196.27,
      "GBP/AUD": gbpRates["AUD"] ?? 1.9448,
      // AUD/JPY (derived: AUD/USD * USD/JPY)
      "AUD/JPY": (usdRates["AUD"] ?? 0.6538) * (usdRates["JPY"] ?? 154.32),
    };

    // Gold: use a metal price API fallback (Gold is not on Frankfurter)
    // We'll use metals.dev free API
    let goldPrice = 2658.43; // fallback
    try {
      ...
      if (goldRes.ok) {
        const goldData = (await goldRes.json()) as { rates?: Record<string, number> };
        if (goldData.rates?.XAU) {
          // API returns USD per XAU (e.g., 0.000376), we need XAU per USD
          goldPrice = goldData.rates.XAU > 1 ? goldData.rates.XAU : 1 / goldData.rates.XAU;
        }
      }
    } catch {
      // Gold price API failed — use fallback
      console.warn("[Forex] Gold price API failed, using fallback");
    }

    const result: ForexPriceItem[] = FOREX_PAIR_CONFIG.map((cfg) => ({
      pair: cfg.pair,
      name: cfg.name,
      price: cfg.type === "gold" ? goldPrice : (priceMap[cfg.pair] ?? 0),
      type: cfg.type,
      badge: cfg.badge,
    }));

    forexCache = { data: result, fetchedAt: Date.now() };
    return result;
  } catch (error) {
    console.warn("[Forex] Failed to fetch live prices:", error);
    // Return fallback data
    return FOREX_PAIR_CONFIG.map((cfg) => ({
      pair: cfg.pair,
      name: cfg.name,
      price: 0,
      type: cfg.type,

...
function detectGartley(pattern: XABCD): ChartFormation | null {
  ...
  const przTarget = isBullish ? D.price + AD * 0.618 : D.price - AD * 0.618;

  const stopPrice = isBullish ? X.price - XA * 0.05 : X.price + XA * 0.05;
...
function detectButterfly(pattern: XABCD): ChartFormation | null {
  ...
  const stopPrice = isBullish ? D.price - XA * 0.05 : D.price + XA * 0.05;
...
function detectCrab(pattern: XABCD): ChartFormation | null {
  ...
  const przTarget = isBullish ? D.price + AD * 0.382 : D.price - AD * 0.382;
...
function detectShark(pattern: XABCD): ChartFormation | null {
  ...
  const stopPrice = isBullish ? D.price - BC * 0.05 : D.price + BC * 0.05;
...
function detectABCD(pattern: XABCD): ChartFormation | null {
  ...
  const przTarget = isBullish ? D.price + AB * 0.618 : D.price - AB * 0.618;

  const stopPrice = isBullish ? D.price - AB * 0.05 : D.price + AB * 0.05;

...
/**
 * Server function to search tokens by symbol or contract address directly via DexScreener Search API.
 * Supports contract addresses on Solana, Ethereum, Base, BSC, Arbitrum, Polygon, etc.
 */
export const searchDexTokens = createServerFn({ method: "GET" })
  .validator((q: string) => q)
  .handler(async ({ data: query }): Promise<EnrichedToken[]> => {
    ...
    try {
      ...
      const tokens: EnrichedToken[] = data.pairs.slice(0, 20).map((p: any) => ({

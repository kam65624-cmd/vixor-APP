...
export async function generateWeeklyReport(userId: string): Promise<AnalystReport> {
  return {
    ...
    behavioralPatterns:
      "Tends to overtrade during high volatility sessions. Better performance during Asian session.",
    recommendations:
      "Focus on fewer, higher-conviction trades. Maintain trading journal consistency.",

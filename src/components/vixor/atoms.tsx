...
import { TrendingUp, TrendingDown, Minus, Shield, ShieldAlert, ShieldQuestion, ChevronDown, BookOpen } from "lucide-react";
...
export function SetupStrengthBadge({ strength, size = "md" }: { strength: SetupStrength; size?: "sm" | "md" | "lg" }) {
  const config = {
    STRONG: {
      label: "Strong Setup",
      icon: Shield,
...
export function highlightSMCTerms(text: string): React.ReactNode[] {
  ...
  return parts.map((part, i) => {
    if (terms.some(t => t.toLowerCase() === part.toLowerCase())) {
      return <span key={i} className="term-highlight" title={SMC_GLOSSARY[terms.find(t => t.toLowerCase() === part.toLowerCase())!]}>{part}</span>;
    }
    return part;
  });
}
  ...
  return (
    <div className="terminal-card p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">SMC Glossary</span>
      </div>
...
export function PriceCell({ label, value, accent }: { label: string; value: string | number; accent?: "bullish" | "bearish" | "neutral" | "muted" }) {
  const colorMap = {

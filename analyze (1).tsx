...
import { Upload, Image as ImageIcon, Camera, Clipboard, Calculator, Sparkles, X, Loader2 } from "lucide-react";
...
const STEPS = ["Validating chart", "Analyzing patterns", "Generating trade setup", "Finalizing results"];
...
function Analyze() {
  ...
  async function startAnalysis() {
    ...
    if (!isPremium && points < 10) { setErr("Need at least 10 points"); return; }
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Drop any chart screenshot. We'll do the rest.</p>
      </div>
      ...
      {stage === "upload" && (
        <>
          ...
          <div className="grid grid-cols-3 gap-2">
            <button onClick={() => fileRef.current?.click()} className="vixor-card vixor-card-hover p-3 flex flex-col items-center gap-2">
              <ImageIcon className="size-5 text-muted-foreground" /><span className="text-xs font-medium">Gallery</span>
            </button>
            <button onClick={() => { if (fileRef.current) { fileRef.current.setAttribute("capture", "environment"); fileRef.current.click(); }}} className="vixor-card vixor-card-hover p-3 flex flex-col items-center gap-2">
              <Camera className="size-5 text-muted-foreground" /><span className="text-xs font-medium">Camera</span>
            </button>
            <button onClick={handlePaste} className="vixor-card vixor-card-hover p-3 flex flex-col items-center gap-2">
              <Clipboard className="size-5 text-muted-foreground" /><span className="text-xs font-medium">Paste</span>
            </button>
          </div>
          ...
          <div className="vixor-card p-4 flex items-center gap-3">
            ...
            <button onClick={() => navigate({ to: "/lot-calculator" })} className="text-xs text-primary font-semibold">Open</button>
          </div>

          {err && <div className="text-xs text-bearish text-center">{err}</div>}
          <div className="text-xs text-muted-foreground text-center">
            {isPremium ? "Premium · unlimited analyses" : `Each analysis costs 10 points · You have ${points} pts`}
          </div>
        </>
      )}
        ...
        <>
          <div className="vixor-card overflow-hidden">
            <div className="aspect-[4/3] bg-card-hover flex items-center justify-center relative">
              ...
              <button onClick={() => { setStage("upload"); setPreview(null); setFile(null); }} className="absolute top-2 right-2 size-8 rounded-full bg-background/80 backdrop-blur flex items-center justify-center">
                <X className="size-4" />
              </button>
            </div>
            <div className="p-3 text-xs text-muted-foreground truncate">{file?.name} · {file ? Math.round(file.size / 1024) : 0} KB</div>
          </div>
          {err && <div className="text-xs text-bearish text-center">{err}</div>}

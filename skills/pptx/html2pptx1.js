...
const COMPENSATION = {
  HEADING_WIDTH: 0.1,
  SINGLE_LINE_NARROW: 0.1,
  SINGLE_LINE_NORMAL: 0.1,
  MULTI_LINE: 0.05,
  SHORT_TEXT_EXTRA: 0.18,
  ...
  VERY_SHORT_TEXT_EXTRA: 0.3,
  NOWRAP_EXTRA: 0.15,
  MAX_WIDTH_FACTOR: 0.6,
  TEXT_HEIGHT: 0.08,
  LIST_HEIGHT: 0.06,
  MIN_FONT_SIZE_PT: 11,
  VERTICAL_BALANCE_THRESHOLD: 0.65,
...
function getNarrowBodyTextClipSlack(el) {
  ...
  return Math.min(0.1, Math.max(0.04, el.position.w * 0.06));
}
...
function hasExplicitTextBreaks(el) {
  ...
  if (Array.isArray(el.text))
    return el.text.some((r) => (r.text && r.text.includes("\n")) || r.options?.breakLine);
  return false;
}
...
function calculateWidthCompensation(el, slideWidthIn) {
  ...
  const noExplicitBreaks =
    ...
    (Array.isArray(el.text) &&
      !el.text.some((r) => (r.text && r.text.includes("\n")) || r.options?.breakLine));
  ...
  const compactTxt = txt.replace(/\s+/g, "");
  const isShortText =
    compactTxt.length > 0 && compactTxt.length <= COMPENSATION.AUTO_SHORT_TEXT_THRESHOLD;
    ...
    if (single && !el.noWrap && fsHere > 0) {
      ...
      if (ratio > 0.08) {
        ...
        if (boxWidthPt > 0 && estimateTextWidthPt(txt, fsHere) <= boxWidthPt * 1.3) {
          ...
          f0 = Math.max(f0, Math.min(0.3, ratio * 0.9 * lenBoost));
        }
      }
    }
    return f0;
  ...
  let f = isH
    ...
    : el.position.w < slideWidthIn / 3 && txt.length < 14
  ...
  const isItalic =
    el.style?.italic === true || (Array.isArray(el.text) && el.text.some((r) => r.options?.italic));
...
const hasEmojiCharsNode = (text = "") =>
  /[\u2600-\u27BF]|[\uD800-\uDBFF][\uDC00-\uDFFF]/.test(text);
const hasIconSymbolCharsNode = (text = "") =>
  /[\u2190-\u21FF\u2300-\u23FF\u2900-\u297F\u2460-\u24FF\u25A0-\u27BF\u2B00-\u2BFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]/.test(
    text,
  );
...
function shouldUseNumericEmphasisText(text = "") {
  ...
  const labelPrefix =
    /^(?:lv|no|honor|resource|step|part|section|chapter|page|slide|p|q|a|item|case|fig|table)\b/i.test(
      compact,
    );
...
function applyEmphasisFont(slideData, fontConfig = DEFAULT_FONT_CONFIG) {
  const cfg =
    typeof fontConfig === "string"
      ? { ...DEFAULT_FONT_CONFIG, emphasis: fontConfig }
  ...
  for (const el of slideData.elements) {
    ...
    if (typeof el.text === "string") {
      ...
      for (const run of el.text) {
        ...
        if (!trimmed) continue;
        if (hasEmojiCharsNode(trimmed) && Array.from(trimmed).length <= 4)
          run.options.fontFace = cfg.symbol;
...
async function getBodyDimensions(page) {
  const bodyDimensions = await page.evaluate(() => {
    ...
    const rootW = Math.max(
      rect.width || 0,
      slideRoot.scrollWidth || 0,
      parseFloat(style.width) || 0,
    );
    const rootH = Math.max(
      rect.height || 0,
      slideRoot.scrollHeight || 0,
  ...
  if (widthOverflowPt > OVERFLOW_TOLERANCE_PT || heightOverflowPt > OVERFLOW_TOLERANCE_PT) {
    const directions = [];
    if (widthOverflowPt > OVERFLOW_TOLERANCE_PT)
      directions.push(`${widthOverflowPt.toFixed(1)}pt horizontally`);
    if (heightOverflowPt > OVERFLOW_TOLERANCE_PT)
      directions.push(`${heightOverflowPt.toFixed(1)}pt vertically`);
    const reminder =
      heightOverflowPt > OVERFLOW_TOLERANCE_PT ? ' (leave 0.5" margin at bottom)' : "";
    if (maxOverflowPt > OVERFLOW_SEVERE_PT) {
      // Severe: spilled by >1in — content is being clipped, MUST be fixed.
      errors.push(
        `🚨🚨 CRITICAL OVERFLOW (MUST FIX): HTML content overflows body by ${directions.join(" and ")}${reminder} — content is being clipped; reduce content / font size or restructure the source HTML and re-run.`,
      );
    } else {
      errors.push(
        `🚨 CRITICAL OVERFLOW: HTML content overflows body by ${directions.join(" and ")}${reminder}`,
      );
    }
  }
...
function addElements(slideData, targetSlide, pres, tmpDir) {
  ...
  for (const el of sortedElements) {
    if (el.type === "image") {
      ...
      let imagePath =
        el.src && el.src.startsWith("file://") ? el.src.replace("file://", "") : el.src;
      ...
      if (typeof imagePath === "string" && imagePath.startsWith("data:image/svg+xml")) {
        console.warn(
          "[html2pptx] dropping un-rasterized SVG data URL — sharp may have failed earlier",
        );
        continue;
      }
      const opts = {
      ...
      try {
        ...
        const isLocalFile =
          typeof imagePath === "string" && !isDataUrl && !/^https?:\/\//i.test(imagePath);
      ...
      let adjustedH =
        el.position.h * (1 + COMPENSATION.LIST_HEIGHT) + getListSpacingHeightBonus(el);
      ...
      const centerInFrame =
        el.style?._centerInFrame || (iconOnlyText && !el.style?._inlineLeadGlyph);
      ...
      const widthFactor =
        centerInFrame || hasContainerPadding || isRotatedText
          ? 0
          : Math.max(
              calculateWidthCompensation(el, slideWidthIn),
              isHeadingWithBreaks ? COMPENSATION.HEADING_WIDTH : 0,
            );
      const widthIncrease = el.position.w * widthFactor;
      ...
      if (
        !isRotatedText &&
        !el.style?._inlineLeadGlyph &&
        Number.isFinite(preferredRightBound) &&
      ...
      let adjustedH = centerInFrame
        ? el.position.h
        : el.position.h * (1 + COMPENSATION.TEXT_HEIGHT) * clampRatio;
      ...
      if (clip) {
        const clipRight =
          clip.x + clip.w + getNarrowBodyTextClipSlack(el) + getMediumSingleLineClipSlack(el);
        if (adjustedX < clip.x) {
        ...
        if (adjustedX + adjustedW > clipRight) {
          ...
          const shiftLeft = canShiftLeft
            ? Math.min(overflowRight, Math.max(0, adjustedX - clip.x))
            : 0;
          adjustedX -= shiftLeft;
          adjustedW -= overflowRight - shiftLeft;
        }
        if (el.position.y + adjustedH > clip.y + clip.h)
          adjustedH = clip.y + clip.h - el.position.y;
      }
          ...
          if (_estPt2 > _curPt * 0.72) {
            ...
            if (_needIn > adjustedW) {
              ...
              if (adjustedX + adjustedW > slideWidthIn)
                adjustedX = Math.max(0, slideWidthIn - adjustedW);
            }
          }
        }
      }
      ...
      if (
        !el.style?._allowWrap &&
        (el.noWrap || isShortAutoNoWrapText(el) || (isHeadingWithBreaks && !headingBreaksOverflow))
      )
...
async function extractSlideData(page, slideDims) {
  return await page.evaluate((slideDims) => {
    ...
    const bodyWPx =
      slideRootRect.width || slideRoot.scrollWidth || document.body.clientWidth || 1280;
    const bodyHPx =
      slideRootRect.height || slideRoot.scrollHeight || document.body.clientHeight || 720;
    ...
    const FONT_FALLBACK_MAP = {
      ...
      "lxgw wenkai": "KaiTi",
      "lxgw wenkai tc": "KaiTi",
      "lxgw wenkai gb": "KaiTi",
      霞鹜文楷: "KaiTi",
      "霞鹜文楷 tc": "KaiTi",
      fredoka: "Arial Rounded MT Bold",
      "fredoka one": "Arial Rounded MT Bold",
      "system-ui": "Carlito",
      "-apple-system": "Carlito",
      blinkmacsystemfont: "Carlito",
      "segoe ui": "Carlito",
      ...
      nunito: "Carlito",
      "nunito sans": "Carlito",
      "source sans pro": "Carlito",
      "source sans 3": "Carlito",
      "source serif pro": "Tinos",
      "work sans": "Carlito",
      "dm sans": "Carlito",
      "space grotesk": "Carlito",
      "plus jakarta sans": "Carlito",
      manrope: "Carlito",
      "fira sans": "Carlito",
      "playfair display": "Tinos",
      merriweather: "Tinos",
      "libre baskerville": "Tinos",
      "pt sans": "Carlito",
      "pt serif": "Tinos",
      ubuntu: "Carlito",
      ...
      "century gothic": "Carlito",
      "palatino linotype": "Tinos",
      palatino: "Tinos",
      "trebuchet ms": "Carlito",
      garamond: "Tinos",
      rockwell: "Tinos",
      candara: "Carlito",
      corbel: "Carlito",
      ...
      "bodoni mt": "Tinos",
      "copperplate gothic": "Carlito",
      "tw cen mt": "Carlito",
      "century schoolbook": "Tinos",
      "book antiqua": "Tinos",
      // Condensed / display sans → Liberation Sans Narrow (closer geometry)
      "bebas neue": "Liberation Sans Narrow",
      "bebas neue pro": "Liberation Sans Narrow",
      oswald: "Liberation Sans Narrow",
      anton: "Liberation Sans Narrow",
      "fjalla one": "Liberation Sans Narrow",
      teko: "Liberation Sans Narrow",
      "saira condensed": "Liberation Sans Narrow",
      saira: "Carlito",
      "roboto condensed": "Liberation Sans Narrow",
      "archivo narrow": "Liberation Sans Narrow",
      "barlow condensed": "Liberation Sans Narrow",
      "barlow semi condensed": "Liberation Sans Narrow",
      "encode sans condensed": "Liberation Sans Narrow",
      "pt sans narrow": "Liberation Sans Narrow",
      "league spartan": "Liberation Sans Narrow",
      "league gothic": "Liberation Sans Narrow",
      "six caps": "Liberation Sans Narrow",
      "big shoulders display": "Liberation Sans Narrow",
      "big shoulders text": "Liberation Sans Narrow",
      "alumni sans": "Liberation Sans Narrow",
      "dharma gothic e": "Liberation Sans Narrow",
      "titillium web": "Liberation Sans Narrow",
      staatliches: "Liberation Sans Narrow",
      bungee: "Liberation Sans Narrow",
      "bungee shade": "Liberation Sans Narrow",
      "bungee inline": "Liberation Sans Narrow",
      orbitron: "Liberation Sans Narrow",
      rajdhani: "Liberation Sans Narrow",
      // Pixel / retro fonts — none ship with PPT; map to a monospaced
      // pass-through so glyphs at least keep a uniform Latin width.
      vt323: "Liberation Mono",
      "press start 2p": "Liberation Mono",
      "major mono display": "Liberation Mono",
      silkscreen: "Liberation Mono",
      "pixelify sans": "Liberation Mono",
      "jersey 10": "Liberation Mono",
      ...
      sora: "Carlito",
      urbanist: "Carlito",
      "be vietnam pro": "Carlito",
      rubik: "Carlito",
      archivo: "Carlito",
      "instrument sans": "Carlito",
      "instrument serif": "Tinos",
      "cormorant garamond": "Tinos",
      "eb garamond": "Tinos",
      lora: "Tinos",
      "crimson text": "Tinos",
      "crimson pro": "Tinos",
      ...
      onest: "Carlito",
      satoshi: "Carlito",
      "general sans": "Carlito",
      switzer: "Carlito",
      syne: "Carlito",
      "red hat display": "Carlito",
      "red hat text": "Carlito",
      "public sans": "Carlito",
      "reddit sans": "Carlito",
      mulish: "Carlito",
      "hanken grotesk": "Carlito",
      karla: "Carlito",
      outfit: "Carlito",
      "bricolage grotesque": "Carlito",
      "host grotesk": "Carlito",
      unbounded: "Carlito",
      "ibm plex sans": "Carlito",
      "ibm plex serif": "Tinos",
      "noto sans": "Carlito",
      "noto serif": "Tinos",
      "libre franklin": "Carlito",
      oswald: "Liberation Sans Narrow",
      barlow: "Carlito",
      "barlow condensed": "Liberation Sans Narrow",
      epilogue: "Carlito",
      ...
      "pt serif": "Tinos",
      cardo: "Tinos",
      "old standard tt": "Tinos",
      "noto serif display": "Tinos",
      tinos: "Tinos",
      "big shoulders display": "Liberation Sans Narrow",
      "roboto serif": "Tinos",
      "dm serif display": "Tinos",
      "dm serif text": "Tinos",
      ...
      "departure mono": "Courier New",
      inconsolata: "Courier New",
      "commit mono": "Courier New",
      "martian mono": "Courier New",
      // CJK display fonts that aren't in PPT — map to Microsoft YaHei
      "smiley sans": "Microsoft YaHei",
      "smiley sans oblique": "Microsoft YaHei",
      "lxgw wenkai screen": "KaiTi",
      "zcool kuaile": "KaiTi",
      "zcool xiaowei": "SimSun",
      "zcool qingke huangyou": "KaiTi",
      "long cang": "KaiTi",
      "ma shan zheng": "KaiTi",
      ...
      "harmonyos sans": "Microsoft YaHei",
      "harmonyos sans sc": "Microsoft YaHei",
      opposans: "Microsoft YaHei",
      "oppo sans": "Microsoft YaHei",
      misans: "Microsoft YaHei",
      "mi sans": "Microsoft YaHei",
      小米兰亭: "Microsoft YaHei",
      "xiaomi lanting": "Microsoft YaHei",
      霞鹜文楷: "KaiTi",
      "lxgw wenkai gb": "KaiTi",
      阿里妈妈数黑体: "Microsoft YaHei",
      "alimama shuheiti": "Microsoft YaHei",
      "noto serif hk": "SimSun",
      "noto sans hk": "Microsoft YaHei",
      "noto sans tc": "Microsoft YaHei",
      "noto serif tc": "SimSun",
      "noto sans jp": "Microsoft YaHei",
      "noto serif jp": "SimSun",
      "noto sans kr": "Microsoft YaHei",
      "noto serif kr": "SimSun",
      ...
      文泉驿微米黑: "Microsoft YaHei",
      文泉驿正黑: "Microsoft YaHei",
      "adobe heiti std": "Microsoft YaHei",
      "adobe fangsong std": "KaiTi",
      "adobe kaiti std": "KaiTi",
      "adobe song std": "SimSun",
      "ar pl ukai cn": "KaiTi",
      "ar pl uming cn": "SimSun",
      fzhei: "Microsoft YaHei",
      fzkai: "KaiTi",
      fzsong: "SimSun",
      方正黑体: "Microsoft YaHei",
      方正楷体: "KaiTi",
      方正宋体: "SimSun",
      方正书宋: "SimSun",
      方正仿宋: "KaiTi",
      汉仪旗黑: "Microsoft YaHei",
      汉仪文黑: "Microsoft YaHei",
      hyqihei: "Microsoft YaHei",
      hywenhei: "Microsoft YaHei",
      ruiziyun: "Microsoft YaHei",
      锐字: "Microsoft YaHei",
      "dingtalk jinbuti": "Microsoft YaHei",
      钉钉进步体: "Microsoft YaHei",
      youshebiaotihei: "Microsoft YaHei",
      优设标题黑: "Microsoft YaHei",
      pangmen: "Microsoft YaHei",
      庞门正道标题体: "Microsoft YaHei",
      aocomic: "KaiTi",
      maoken: "KaiTi",
      "tw-kai": "KaiTi",
      mingliu: "SimSun",
      pmingliu: "SimSun",
      // Korean web fonts
      "noto sans korean": "Microsoft YaHei",
      "noto serif korean": "SimSun",
      "spoqa han sans": "Microsoft YaHei",
      "spoqa han sans neo": "Microsoft YaHei",
      "nanum gothic": "Microsoft YaHei",
      "nanum myeongjo": "SimSun",
      "nanum barun gothic": "Microsoft YaHei",
      "nanum pen script": "KaiTi",
      "gowun batang": "SimSun",
      "gowun dodum": "Microsoft YaHei",
      "do hyeon": "Microsoft YaHei",
      jua: "Microsoft YaHei",
      "black han sans": "Microsoft YaHei",
      "black and white picture": "Microsoft YaHei",
      sunflower: "Microsoft YaHei",
      gaegu: "KaiTi",
      // Japanese display fonts
      "kosugi maru": "Microsoft YaHei",
      kosugi: "Microsoft YaHei",
      "sawarabi gothic": "Microsoft YaHei",
      "sawarabi mincho": "SimSun",
      "m plus 1p": "Microsoft YaHei",
      "m plus rounded 1c": "Microsoft YaHei",
      "m plus 1": "Microsoft YaHei",
      "m plus 2": "Microsoft YaHei",
      "klee one": "KaiTi",
      "shippori mincho": "SimSun",
      "shippori mincho b1": "SimSun",
      "rampart one": "Microsoft YaHei",
      "rocknroll one": "Microsoft YaHei",
      "reggae one": "Microsoft YaHei",
      "dot gothic 16": "Liberation Mono",
      dotgothic16: "Liberation Mono",
      "yusei magic": "Microsoft YaHei",
      "mochiy pop one": "Microsoft YaHei",
      "kaisei opti": "SimSun",
      "kaisei tokumin": "SimSun",
      // Additional modern Google Fonts not yet covered
      "albert sans": "Carlito",
      "cabinet grotesk": "Carlito",
      "clash display": "Liberation Sans Narrow",
      "clash grotesk": "Carlito",
      "general sans": "Carlito",
      supreme: "Carlito",
      chillax: "Carlito",
      erode: "Tinos",
      gambarino: "Tinos",
      melodrama: "Tinos",
      tanker: "Liberation Sans Narrow",
      panchang: "Liberation Sans Narrow",
      array: "Liberation Sans Narrow",
      sentient: "Tinos",
      "familjen grotesk": "Carlito",
      "instrument display": "Tinos",
      "fragment mono": "Courier New",
      redaction: "Tinos",
      "authentic sans": "Carlito",
      "space grotesk variable": "Carlito",
      "national park": "Liberation Sans Narrow",
      "reddit mono": "Courier New",
      "noto sans mono": "Courier New",
      "azeret mono": "Courier New",
      "sometype mono": "Courier New",
      ...
      allura: "Tinos",
      tangerine: "Tinos",
      "dancing script": "Tinos",
      satisfy: "Tinos",
      parisienne: "Tinos",
      "amatic sc": "Carlito",
      "shadows into light": "Tinos",
      caveat: "Tinos",
      kalam: "Tinos",
      "patrick hand": "Tinos",
      "gloria hallelujah": "Tinos",
      "architects daughter": "Tinos",
      "permanent marker": "Tinos",
      "indie flower": "Tinos",
      "homemade apple": "Tinos",
      "rock salt": "Tinos",
    ...
    const heuristicMapFont = (font, cjkText) => {
      ...
      const cleaned = lower
        .replace(
          /\b(thin|extralight|ultralight|light|regular|book|medium|semibold|demibold|bold|extrabold|ultrabold|black|heavy)\b/g,
          " ",
        )
      ...
      if (
        /(\bhei\b|黑|gothic|gothik|sans.*(cn|sc|tc|hk|jp|kr|cjk|chinese|chn|han|hans|hant)|(cn|sc|tc|hk|jp|kr|cjk|chinese|chn|han|hans|hant).*sans)/.test(
          cleaned,
        )
      ) {
      ...
      if (
        /(\bsong\b|宋|\bming\b|明朝|明体|mincho|serif.*(cn|sc|tc|hk|jp|kr|cjk|chinese|han|hans|hant)|(cn|sc|tc|hk|jp|kr|cjk|chinese|han|hans|hant).*serif)/.test(
          cleaned,
        )
      ) {
        return "SimSun";
      ...
      if (
        /(\bkai\b|楷|fangsong|仿宋|xing(kai|shu)?|行楷|行书|li ?bian|隶书|maru|手写|brush|caoshu|草书|手书|hand|script)/.test(
          cleaned,
        )
      ) {
        return "KaiTi";
      ...
      if (
        /(\bmono\b|\bcode\b|console|courier|terminal|typewriter|fixedsys|menlo|monaco|consolas|hack|iosevka|berkeley mono|comic mono)/.test(
          cleaned,
        )
      ) {
        return "Courier New";
      ...
      if (
        /(bebas|oswald|anton|teko|saira|fjalla|tungsten|alfa slab|six caps|big shoulders|archivo narrow|barlow condensed|roboto condensed|encode sans condensed|league spartan|league gothic|alumni sans|dharma gothic)/.test(
          cleaned,
        )
      ) {
        return "Liberation Sans Narrow";
      ...
      if (
        /(\bserif\b|antiqua|times|roman|garamond|georgia|caslon|baskerville|didot|bodoni|jenson|sabon|trajan|minion|palatino|fraktur|book|elephant|venetian)/.test(
          cleaned,
        )
      ) {
        return "Tinos";
      ...
      if (
        /(script|hand|brush|cursive|calligraphic|signature|pen|ink|chalk|marker|kalam|caveat|pacifico|dancing|sacramento|satisfy|allura|great vibes|tangerine|parisienne|amatic|shadows|gloria|architects|patrick|reenie|indie|covered by your grace|nothing you could do|rock salt|permanent marker|homemade|just another hand)/.test(
          cleaned,
        )
      ) {
        return "Tinos";
      ...
      if (
        /(pixel|pixelify|press start|vt323|silkscreen|jersey|retro|8.?bit|terminal)/.test(cleaned)
      ) {
        return "Liberation Mono";
      }
      // Anything explicitly "sans" / "grotesk" / "gothic" / "neue" / common humanist → Carlito
      if (
        /(\bsans\b|\bgrotesk\b|grotesque|gothic|neue|deck|display|text|ui|interface|system|humanist|geometric|neo|standard|modern)/.test(
          cleaned,
        )
      ) {
        return "Carlito";
    ...
    const hasEmojiChars = (text = "") =>
      /[\u2600-\u27BF]|[\uD800-\uDBFF][\uDC00-\uDFFF]/.test(text);
    const hasIconSymbolChars = (text = "") =>
      /[\u2190-\u21FF\u2300-\u23FF\u2900-\u297F\u2460-\u24FF\u25A0-\u27BF\u2B00-\u2BFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]/.test(
        text,
      );
    ...
    const shouldApplyPptBold = (fontWeight) =>
      fontWeight === "bold" || (parseInt(fontWeight, 10) || 0) >= 700;
    const isMediumOrBold = (fontWeight) =>
      fontWeight === "bold" || (parseInt(fontWeight, 10) || 0) >= 600;
    ...
    const appendTextRuns = (runs, text, baseOptions = {}) => {
      ...
      if (whole && !hasCJKChars(whole) && shouldUseEmphasisFont(whole)) {
        pushTextRun(runs, fullText, {
          ...baseOptions,
          fontFace: _fc.emphasis || "Liberation Sans Narrow",
        });
        return;
      }
      if (whole && hasEmojiChars(whole) && Array.from(whole).length <= 4) {
        pushTextRun(runs, fullText, {
          ...baseOptions,
          fontFace: _fc.symbol || "Segoe UI Symbol",
          bold: false,
        });
        return;
      }
      const shouldSplit =
        (hasCJKChars(fullText) &&
          (hasLatinChars(fullText) || hasDigitChars(fullText) || hasEmojiChars(fullText))) ||
        hasEmojiChars(fullText);
      if (!shouldSplit) {
        pushTextRun(runs, fullText, baseOptions);
        return;
      }
      const tokenRe =
        /([\uD800-\uDBFF][\uDC00-\uDFFF]|[A-Za-z]+\.?\d[A-Za-z0-9._:/+%\-]*|[+\-]?\d[\d.,:/+%\-]*(?:[A-Za-z]+)?|[A-Za-z]+(?:[-_][A-Za-z]+)*)/g;
      let last = 0;
      let match;
    ...
    const splitTextIntoFontRuns = (text, baseOptions = {}) => {
      ...
      return runs.length > 1 || (runs[0] && !sameRunOptions(runs[0].options, baseOptions))
        ? runs
        : null;
    };
    ...
    const getMinFontSize = (textContent = "") =>
      hasCJKChars(textContent) ? MIN_CJK_BODY_FONT_SIZE_PT : MIN_FONT_SIZE_PT;
    const pxToFontSize = (pxStr, textContent = "") =>
      Math.max(getMinFontSize(textContent), pxToPoints(pxStr));
    ...
    const extractAlpha = (rgbStr) => {
      ...
      // HSLA
      const hslaMatch = rgbStr.match(
        /hsla\(\s*[\d.]+(?:deg)?\s*,\s*[\d.]+%\s*,\s*[\d.]+%\s*,\s*([\d.]+)\)/,
      );
    ...
    const computeTransparency = (colorStr, opacityStr) => {
      ...
      const opacityT =
        Number.isFinite(cssOpacity) && cssOpacity < 1 ? Math.round((1 - cssOpacity) * 100) : null;
    ...
    const parseLinearGradientLayer = (layer) => {
      const m = String(layer || "")
        .trim()
        .match(/^linear-gradient\((.*)\)$/i);
      if (!m) return null;
      ...
      const stops = stopParts
        .map((part, idx) => {
          const p = part.trim();
          const cm = p.match(
            /^(rgba?\([^)]*\)|hsla?\([^)]*\)|#[0-9a-fA-F]{3,8}|transparent|[a-zA-Z]+)\s*(.*)$/i,
          );
          if (!cm) return null;
          ...
          return {
            color: cm[1],
            pos: posM
              ? Math.max(0, Math.min(1, parseFloat(posM[1]) / 100))
              : stopParts.length === 1
                ? 0
    ...
    const parseRadialGradientLayer = (layer) => {
      ...
      const isConfig =
        (!isColorFirst &&
          /(^|\s)(at|circle|ellipse|closest-side|closest-corner|farthest-side|farthest-corner)\b/i.test(
            first,
          )) ||
      ...
      if (isConfig) {
        ...
        if (atM) {
          cxPct = parseFloat(atM[1]);
          cyPct = parseFloat(atM[2]);
        }
        if (/\bcircle\b/i.test(first)) shape = "circle";
        const sizeM = first.replace(/\bat\b.*$/i, "").match(/([\d.]+)%\s+([\d.]+)%/);
        if (sizeM) {
          rxPct = parseFloat(sizeM[1]);
          ryPct = parseFloat(sizeM[2]);
        }
      }
      const stops = stopParts
        .map((part, idx) => {
      ...
      return stops.length >= 2
        ? { type: "radial", cxPct, cyPct, rxPct, ryPct, shape, stops }
        : null;
    };

    const isFullyTransparentCssColor = (colorStr) => {
      const c = String(colorStr || "")
        .trim()
        .toLowerCase();
      if (!c) return false;
    ...
    const resolveTextPaintStyle = (computed, element = null) => {
      const fillColor =
        computed.webkitTextFillColor && computed.webkitTextFillColor !== "currentcolor"
          ? computed.webkitTextFillColor
          : computed.color;
      const transparentTextPaint =
        isFullyTransparentCssColor(fillColor) || isFullyTransparentCssColor(computed.color);
    ...
    const renderLinearGradientBackground = (bgImage, widthPx, heightPx) => {
      ...
      for (const layer of layers.reverse()) {
        if (layer.type === "radial") {
          ...
          const len =
            Math.abs(canvas.width * vx) + Math.abs(canvas.height * vy) ||
      ...
      element.childNodes.forEach((node) => {
        ...
        if (isText) {
          ...
          if (
            ...
            const parentComputed = node.parentElement
              ? window.getComputedStyle(node.parentElement)
              : null;
            const parentIsRowFlex =
              parentComputed &&
            ...
            if (computed.fontFamily)
              options.fontFace = mapFontFace(computed.fontFamily, node.textContent);
            ...
            const hasInlineBg =
              inlineBg && inlineBg !== "rgba(0, 0, 0, 0)" && inlineBg !== "transparent";
            ...
            if (
              (computed.color && computed.color !== "rgb(0, 0, 0)") ||
              inlineTextPaint._gradientTextFallback
            ) {
            ...
            if (inlineTextPaint.transparency !== null)
              options.transparency = inlineTextPaint.transparency;
            if (computed.fontSize)
              options.fontSize = pxToFontSize(computed.fontSize, node.textContent);
            if (computed.letterSpacing && computed.letterSpacing !== "normal")
              options.charSpacing = pxToPoints(computed.letterSpacing);
            if (isMediumOrBold(computed.fontWeight) && shouldUseEmphasisFont(node.textContent))
              options.fontFace = _fc.emphasis || "Liberation Sans Narrow";
            ...
            if (tag === "SUB" || tag === "SUP" || tag === "SMALL" || tag === "BIG" || tag === "A") {
              ...
              if (node.childNodes.length > 0 && (tag === "A" || tag === "SMALL" || tag === "BIG")) {
                ...
                const txt = textTransform(
                  normalizeWhitespaceForPpt(node.textContent, computed.whiteSpace),
                );
    ...
    const notes = noteElements.map(extractNotesText).filter(Boolean).join("\n");
    ...
    const ICON_CLASS_PATTERN =
      /(?:^|\s)(material-icons|material-symbols-(?:outlined|rounded|sharp)|fa-[a-z0-9-]+|fas|far|fab|fal|fad|bi-[a-z0-9-]+|icon-[a-z0-9-]+)(?:\s|$)/;
    const MATERIAL_ICON_CLASS_PATTERN =
      /(?:^|\s)(material-icons|material-symbols-(?:outlined|rounded|sharp))(?:\s|$)/;
    const isIconElement = (el) => {
      ...
      return (
        ...
        ff.includes("font awesome") ||
        ff.includes("fontawesome") ||
        ff.includes("bootstrap-icons") ||
        ff.includes("feather")
      );
    };
    ...
    const extractIconNameFromClass = (cls) => {
      ...
      if (
        fa &&
        !/^(solid|regular|brands|light|duotone|sharp|thin|stack|spin|pulse|fw|li|lg|sm|xs|2x|3x|4x|5x|6x|7x|8x|9x|10x|inverse|rotate|flip|pull|border)$/.test(
          fa[1],
        )
      ) {
        return fa[1];
    ...
    document.querySelectorAll("h1,h2,h3,h4,h5,h6,p").forEach((parent) => {
      ...
      const blockTextChildren = children.filter(
        (c) =>
          [
            "SPAN",
            "B",
      ...
      if (blockTextChildren.length < 2 || blockTextChildren.length !== textBearingChildren.length)
        return;
    ...
    document.querySelectorAll("*").forEach((parent) => {
      ...
      const blockTextChildren = Array.from(parent.children).filter(
        (c) =>
    ...
    document.querySelectorAll("*").forEach((parent) => {
      ...
      const allInline = textKids.every(
        (c) => INLINE_TEXT_CHILD_TAGS.includes(c.tagName) && !_isBlockDisplay(c),
      );
      if (!allInline) return;
    ...
    const getNextFlowTextTopBound = (el, rect) => {
      ...
      if (
        ...
        !String(pcs.flexDirection || "row").startsWith("column")
      ) {
        return null;
      }
    ...
    document.querySelectorAll("*").forEach((el) => {
      ...
      if (el._isIcon) {
        ...
        const textBearingSiblings = iconParent
          ...
          : [];
        const parentCentersIcon =
          parentComputed &&
          parentRect &&
          (parentComputed.display === "flex" ||
          ...
          (parentComputed.justifyContent === "center" ||
            parentComputed.justifyItems === "center") &&
          textBearingSiblings.length === 0;
        if (parentCentersIcon && parentRect.width > 0 && parentRect.height > 0) {
          const computed = window.getComputedStyle(el);
          const fsPx =
            parseFloat(computed.fontSize) || Math.min(parentRect.width, parentRect.height);
          ...
          try {
            ...
            srcNodes.forEach((srcNode, idx) => {
              ...
              if (cs.fill && cs.fill !== "none" && cs.fill !== "rgba(0, 0, 0, 0)")
                cloneNode.setAttribute("fill", cs.fill);
              if (cs.stroke && cs.stroke !== "none" && cs.stroke !== "rgba(0, 0, 0, 0)")
                cloneNode.setAttribute("stroke", cs.stroke);
            });
      ...
      if (el.tagName === "IMG") {
        ...
        if (rect.width > 0 && rect.height > 0) {
          ...
          if (
            br > 0 &&
            computed.borderRadius.includes("%") &&
            br >= 50 &&
        ...
        if (rect.width > 0 && rect.height > 0) {
          ...
          if (!colWidths) {
            ...
            for (const tr of trList) {
              ...
              if (
                expandedCount > bestExpandedCount ||
                (expandedCount === bestExpandedCount && cells.length > bestCells.length)
              ) {
                bestCells = cells;
          ...
          for (const tr of trList) {
            ...
            const rowOut = cells.map((cell) => {
              ...
              const cellAlign =
                cellComputed.textAlign === "start"
                  ? "left"
                  : cellComputed.textAlign === "end"
                    ? "right"
                    : cellComputed.textAlign;
              const valignMap = { top: "top", middle: "middle", bottom: "bottom" };
              const cellOpts = {
                bold:
                  isHeader ||
                  cellComputed.fontWeight === "bold" ||
                  parseInt(cellComputed.fontWeight) >= 700,
          ...
          if (rows.length > 0) {
            ...
            const borderColor =
              tableComputed.borderTopColor && tableComputed.borderTopColor !== "rgba(0, 0, 0, 0)"
      ...
      const isStandaloneInlineShape =
        inlineAsBlockTags.has(el.tagName) && !el.closest("p,h1,h2,h3,h4,h5,h6,li,ul,ol");
      ...
      if (isContainer) {
        ...
        if (!isTextClippedBackground && bgImage && bgImage !== "none") {
          ...
          const isOtherGradient =
            /^(repeating-)?(linear|radial|conic)-gradient/.test(bgImage) && !isLinear;
          ...
          const gradientDataUrl = renderLinearGradientBackground(
            bgImage,
            hostBgRect.width,
            hostBgRect.height,
          );
          ...
          if (isLinear && !gradientDataUrl) {
            ...
            const colorTokens =
              bgImage.match(/(rgba?\([^)]+\)|hsla?\([^)]+\)|#[0-9a-fA-F]{3,8})/g) || [];
            if (colorTokens.length >= 2) {
              ...
              const repStop =
                opaqueStop && !mixedAlpha ? opaqueStop : stops[Math.floor(stops.length / 2)];
        ...
        if ((hasBg && !isSlideCanvasElement) || hasExportableBorder) {
          ...
          if (rect.width > 0 && rect.height > 0) {
            ...
            if (hasBg || hasUniformBorder) {
              ...
              if (hasUniformBorder) {
                const dashType =
                  computed.borderTopStyle === "dashed"
                    ? "dash"
                    : computed.borderTopStyle === "dotted"
                      ? "dot"
                      : "solid";
                lineSpec = {
              ...
              const ownShapeText = normalizeWhitespaceForPpt(
                el.textContent || "",
                computed.whiteSpace,
              ).trim();
              const shapePadX =
              ...
              const isShortPaddedBadgeShape =
                ownShapeText &&
                ownShapeText.length <= 24 &&
                shapePadX + shapePadY > 0 &&
                rect.height <= 48 &&
                rect.width <= 260;
              ...
              if (isThinRotatedFillBox) {
                ...
                const vals = matrixMatch
                  ? matrixMatch[1].split(",").map((v) => parseFloat(v.trim()))
                  : null;
                ...
                if (fillTransparency !== null && fillTransparency !== undefined)
                  lineEl.transparency = fillTransparency;
      ...
      if (el.tagName === "UL" || el.tagName === "OL") {
        ...
        if (!isOrdered && liElements.length > 0) {
          ...
          const listStyleNone =
            (liStyle.listStyleType === "none" && ulStyle.listStyleType === "none") ||
          ...
          if (listStyleNone) {
            ...
            const hasBefore =
              beforeContent && beforeContent !== "none" && beforeContent !== "normal";
            if (hasBefore) {
        ...
        const alignList =
          taLi === "start"
            ...
            : taLi === "end"
              ? "right"
              : taLi === "left" || taLi === "right" || taLi === "center" || taLi === "justify"
                ? taLi
                : "left";
        ...
        elements.push({
          ...
          style: {
            ...
            paraSpaceAfter: Math.max(
              cssLengthToPoints(computed.marginBottom),
              flexListGapPt,
              liMarginBottomPt,
            ),
      ...
      if (el._directTextOnly) {
        ...
        text = normalizeWhitespaceForPpt(
          directTextNodes.map((n) => n.textContent).join(" "),
          computed.whiteSpace,
        ).trim();
        const rects = [];
      ...
      const isVerticalWM =
        computed.writingMode === "vertical-rl" || computed.writingMode === "vertical-lr";
      let cjkVerticalStack = false;
      if (isVerticalWM && hasCJKChars(text)) {
        const nonSpace = text.replace(/\s/g, "");
        const cjkCount = Array.from(nonSpace).filter((c) => /[一-鿿぀-ヿ가-힯]/.test(c)).length;
      ...
      const isFlexRowTitleText =
        flexTextRightBound !== null &&
        text.length <= 64 &&
        !el.querySelector("br") &&
      ...
      else if (taRaw === "left" || taRaw === "right" || taRaw === "center" || taRaw === "justify")
        alignOut = taRaw;
      else alignOut = "left";
      ...
      const parseTextShadow = (ts) => {
        ...
        return {
          ...
          angle: Math.round(angle),
          opacity:
            colorM && extractAlpha(colorM[0]) !== null
              ? (100 - extractAlpha(colorM[0])) / 100
              : 0.5,
        };
      };
      ...
      const isShortPaddedBadgeText =
        baseStyle._hasContainerPadding &&
        text.length <= 24 &&
        rect.height <= 48 &&
      ...
      const pushInlineBackgroundShapes = () => {
        const inlineNodes = Array.from(
          el.querySelectorAll("span,mark,b,strong,i,em,u,a,code,small,big,label"),
        );
        for (const node of inlineNodes) {
          ...
          if (radiusValue > 0) {
            if (radius.includes("%"))
              rectRadius =
                (Math.min(nodeRect.width, nodeRect.height) * radiusValue) / 100 / PX_PER_IN;
      ...
      if (textIsIconOnly && baseStyle._centerInFrame && rotation === null) {
        ...
        const lineHeightPx =
          computed.lineHeight && computed.lineHeight !== "normal"
            ...
            : fsPx * 1.2;
        const iconBoxPx = Math.max(
          1,
          Math.min(Math.min(rect.width, rect.height), Math.max(fsPx, lineHeightPx)),
        );
        const iconRect = {
      ...
      const hasFormatting =
        !el._directTextOnly &&
        el.querySelector(
          "b, i, u, strong, em, span, br, a, sub, sup, s, del, strike, mark, code, small, big",
        );
      ...
      if (hasFormatting) {
        ...
        const runs = parseInlineFormatting(el, baseRunOptions, [], (str) =>
        ...
        if (isMediumOrBold(computed.fontWeight) && shouldUseEmphasisFont(transformedText))
          plainStyle.fontFace = _fc.emphasis || "Liberation Sans Narrow";
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    try {
      ...
      try {
        await page.evaluate(async () => {
          const hasTailwindCdn = Array.from(document.scripts || []).some((s) =>
            /cdn\.tailwindcss\.com/i.test(s.src || ""),
          );
          ...
          const tailwindLooksActive = () => {
            ...
            for (const el of nodes) {
              ...
              if (tokens.includes("-translate-y-1/2") && cs.transform && cs.transform !== "none")
                return true;
            }
            return false;
          };
            ...
            .waitForFunction(
              () => {
                ...
                return cs.every((c) => {
                  ...
                  try {
                    return c.toDataURL("image/png").length > 2000;
                  } catch (e) {
                    return true;
                  }
                });

  ...
  series: [
    {
      ...
      label: {
        ...
        fontSize: 11,
        color: "#6B7280",
        formatter: (p) => (p.dataIndex === 2 ? "{hl|" + p.value + "}" : p.value),
...
function splitTree(data) {
  ...
  const sorted = children.map((c, i) => ({ c, w: countNodes(c), i })).sort((a, b) => b.w - a.w);
  const left = [],
    right = [];
  let lw = 0,
    rw = 0;

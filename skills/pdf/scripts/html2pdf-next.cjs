...
async function convert(inputFile, outputFile, customCSS, options = {}) {
  ...
  try {
    ...
    if (continuousInfo) {
      ...
      if (pagedInfo) {
        console.log(
          `  ✓ Paged.js: ${pagedInfo.pageCount} pages @ ${pagedInfo.pageWidth}×${pagedInfo.pageHeight}px`,
        );
      }
      ...
      const bgFix = await page.evaluate(() => {
        ...
        const pageBg = !isTransparent(bodyBg) ? bodyBg : !isTransparent(htmlBg) ? htmlBg : "white";
      ...
      if (pagedInfo) {
        ...
        if (alignFix.fixed && (alignFix.x > 1 || alignFix.y > 1)) {
          console.log(
            `  ⚠ Page offset after reset: (${alignFix.x}, ${alignFix.y}) — may still have internal pagedjs margins`,
          );
        }
      }
    ...
    const engineLabel = continuousInfo
      ? "Playwright (Creative canvas)"
      : noPaged
        ? "Playwright (Chromium native @page)"
        : "Playwright + Paged.js";

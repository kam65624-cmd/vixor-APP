  ...
  return (
    <div className="space-y-5">
      ...
      <Section title="Notifications">
        ...
        <Row icon={Smartphone} label="Haptics">
...
function Toggle({ on, onChange }: { on: boolean; onChange?: (v: boolean) => void }) {
  return (
    <button
      onClick={() => onChange?.(!on)}
      className={`w-10 h-6 rounded-full transition-colors ${on ? "bg-primary" : "bg-muted"} relative`}
    >
      <div
        className={`absolute top-0.5 size-5 rounded-full bg-background shadow transition-all ${on ? "left-[18px]" : "left-0.5"}`}
      />
    </button>
  );
}
